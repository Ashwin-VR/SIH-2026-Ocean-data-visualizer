...
const cache=new Map<string,unknown>(); const pending=new Map<string,Promise<unknown>>(); let cacheHits=0
export const getApiCacheHits=()=>cacheHits
export const clearApiCache=()=>{cache.clear();pending.clear();cacheHits=0}
async function request<T>(path:string):Promise<T>{const key=`${API_BASE}${path}`;if(cache.has(key)){cacheHits++;return cache.get(key) as T};const active=pending.get(key);if(active)return active as Promise<T>;const task=(async()=>{const r=await fetch(key);if(!r.ok){const b=await r.json().catch(()=>({}));throw new Error(b?.error?.message??`Request failed: ${r.status}`)}const data=await r.json() as T;cache.set(key,data);return data})();pending.set(key,task);try{return await task}finally{pending.delete(key)}}
...
export const getCube=(field:string,b:{latMin:number;latMax:number;lonMin:number;lonMax:number},lod=1,timeIndex?:number)=>request<VolumeResponse>(`/api/fields/${field}/cube?lod=${lod}${timeIndex==null?'':`&time_index=${timeIndex}`}&lat_min=${b.latMin}&lat_max=${b.latMax}&lon_min=${b.lonMin}&lon_max=${b.lonMax}`)
