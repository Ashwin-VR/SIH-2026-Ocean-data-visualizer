...
def open_cf_scalar_field(path: str | Path, variable: str) -> ScalarField:
    ...
    with xr.open_dataset(path) as dataset:
        ...
        depth_dim = next((dims[key] for key in ("depth", "pres", "lev", "z") if key in dims), None)
        ...
        valid_time = str(dataset.coords.get("time").values) if "time" in dataset.coords else "unknown"
        return ScalarField(
